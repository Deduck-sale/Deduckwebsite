export default function Footer() {
  return (
    <footer
      id="contact"
      className="border-t border-white/10 pt-16 pb-8 mt-auto relative z-10 bg-black/20 backdrop-blur-lg"
    >
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex flex-col md:flex-row justify-between items-center md:items-start gap-8">
          <div className="text-center md:text-left">
            <div className="font-bold text-3xl tracking-wider text-white flex items-center justify-center md:justify-start mb-4 drop-shadow-md">
              DE
              <span className="text-deduck-yellow text-4xl leading-none mx-1">
                🦆
              </span>
              UCK
            </div>
            <p className="text-gray-300 max-w-xs font-light">
              Digital Marketing Agency
              <br />
              ทีมงานพร้อม บริการด้วยใจ เก็บทุกความทรงจำ
            </p>
          </div>

          <div className="text-center md:text-right">
            <h4 className="text-white font-semibold text-lg mb-4 tracking-wide">
              ติดต่อเรา (Contact Us)
            </h4>
            <div className="flex flex-col items-center md:items-end space-y-4">
              <a
                href="https://www.facebook.com/deduck.th"
                target="_blank"
                rel="noopener noreferrer"
                className="flex items-center text-gray-300 hover:text-deduck-yellow transition group"
              >
                <div className="w-8 h-8 rounded-full bg-white/5 border border-white/10 flex items-center justify-center mr-3 group-hover:bg-deduck-yellow/20 group-hover:border-deduck-yellow/50 transition">
                  <svg
                    className="w-4 h-4"
                    fill="currentColor"
                    viewBox="0 0 24 24"
                  >
                    <path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.469h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.469h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z" />
                  </svg>
                </div>
                Deduck.th
              </a>
              <a
                href="https://www.instagram.com/deduck.th"
                target="_blank"
                rel="noopener noreferrer"
                className="flex items-center text-gray-300 hover:text-deduck-yellow transition group"
              >
                <div className="w-8 h-8 rounded-full bg-white/5 border border-white/10 flex items-center justify-center mr-3 group-hover:bg-deduck-yellow/20 group-hover:border-deduck-yellow/50 transition">
                  <svg
                    className="w-4 h-4"
                    fill="currentColor"
                    viewBox="0 0 24 24"
                  >
                    <path d="M12 2.163c3.204 0 3.584.012 4.85.07 3.252.148 4.771 1.691 4.919 4.919.058 1.265.069 1.645.069 4.849 0 3.205-.012 3.584-.069 4.849-.149 3.225-1.664 4.771-4.919 4.919-1.266.058-1.644.07-4.85.07-3.204 0-3.584-.012-4.849-.07-3.26-.149-4.771-1.699-4.919-4.92-.058-1.265-.07-1.644-.07-4.849 0-3.204.013-3.583.07-4.849.149-3.227 1.664-4.771 4.919-4.919 1.266-.057 1.645-.069 4.849-.069zM12 0C8.741 0 8.333.014 7.053.072 2.695.272.273 2.69.073 7.052.014 8.333 0 8.741 0 12c0 3.259.014 3.668.072 4.948.2 4.358 2.618 6.78 6.98 6.98C8.333 23.986 8.741 24 12 24c3.259 0 3.668-.014 4.948-.072 4.354-.2 6.782-2.618 6.979-6.98.059-1.28.073-1.689.073-4.948 0-3.259-.014-3.667-.072-4.947-.196-4.354-2.617-6.78-6.979-6.98C15.668.014 15.259 0 12 0zm0 5.838a6.162 6.162 0 100 12.324 6.162 6.162 0 000-12.324zM12 16a4 4 0 110-8 4 4 0 010 8zm6.406-11.845a1.44 1.44 0 100 2.881 1.44 1.44 0 000-2.881z" />
                  </svg>
                </div>
                @deduck.th
              </a>
              <a
                href="https://line.me/R/ti/p/@deduck.th"
                target="_blank"
                rel="noopener noreferrer"
                className="flex items-center text-gray-300 hover:text-deduck-yellow transition group"
              >
                <div className="w-8 h-8 rounded-full bg-white/5 border border-white/10 flex items-center justify-center mr-3 group-hover:bg-[#00B900]/20 group-hover:border-[#00B900]/50 transition text-[#00B900]">
                  <svg
                    className="w-4 h-4"
                    fill="currentColor"
                    viewBox="0 0 24 24"
                  >
                    <path d="M24 10.304c0-5.369-5.383-9.738-12-9.738-6.616 0-12 4.369-12 9.738 0 4.814 3.963 8.898 9.422 9.605.385.083.91.254 1.042.593.118.303.038.777-.035 1.096-.089.387-.43 1.693-.526 2.05-.118.441-.184.887.26 1.066.445.179.914-.067 1.258-.293.435-.286 4.908-2.902 7.158-5.289 2.213-2.333 3.421-5.186 3.421-8.826z" />
                  </svg>
                </div>
                @deduck.th
              </a>
            </div>
          </div>
        </div>

        <div className="mt-12 pt-8 border-t border-white/10 flex flex-col md:flex-row justify-between items-center text-sm text-gray-500">
          <p>&copy; {new Date().getFullYear()} Deduck Agency Co.,Ltd. All rights reserved.</p>
          <p className="mt-2 md:mt-0 font-light">
            Enhance{" "}
            <span className="text-deduck-yellow font-bold tracking-wider">
              YOUR BUSINESS
            </span>
          </p>
        </div>
      </div>
    </footer>
  );
}
